...
if submitted:
    if age != 0 and bmi != 0.0:
        ...
        with open("medical_col\medical_model (1).dat", "rb") as f:
            model = pickle.load(f)
        ...
        # Display prediction
        st.markdown("<div class='prediction'><h3>Prediction Result is</h3><p>" + format_output(output[0]) + "</p></div>", unsafe_allow_html=True)
    else:
